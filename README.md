...
## Пакет для ROS2 `jazzy`/`humble`  

Я интегрировал этот фильтр под `ros2`, все так же на `python`.  

### Установка:  
```bash
cp -r lidar_filter/ ../ros2_ws/src  
cd  
cd ros2_ws  
colcon build --symlink-install --packages-select lidar_filter  
```

### Настройка:  
Конфиг файл лежит в дириктории `configs`.  

| Параметр | Тип | Значение по умолчанию | Описание |
| :--- | :---: | :---: | :--- |
| **frame_id** | `string` | `"laser"` | `frame_id` вашего лидара |
| **scan_topic** | `string` | `"/scan"` | Топик сырого скана |
| **new_scan_topic** | `string` | `"/new_scan"` | Топик нового скана |
| **scan_len** | `int` | `720` | Количество оучей в топик **scan_topic** |
| **min_len** | `int` | `3` | Минимальная длина сегмента (в лучах) для валидации объекта |
| **min_amp** | `float` | `2.5` | Минимальный порог амплитуды сигнала |
| **smooth_win** | `int` | `15` | Размер окна сглаживания данных (в количестве лучей) |
| **stdv_thresh** | `float` | `175` | Порог среднеквадратичного отклонения |
| **max_chng** | `float` | `10000.0` | Максимальное изменение в мм между соседними измерениями |
| **min_var_ratio** | `float` | `0.8` | Минимальный порог отношения дисперсий |

### Запуск:  
```bash
source install/setup.bash  
ros2 launch lidar_filter launch_filter.launch.py  
```

### Демо:  

![ros2](images/ros2.jpg)  
